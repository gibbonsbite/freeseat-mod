...
function get_seat($id) {
  if ($zou = mysql_query("select *,1 as cnt from seats where id=$id"))
...
/** Attempts to lock the given seat for the given show. Returns an
    ...
    This function replaces the locking mecanism used in previous versions,
...
function lock_seats($seat,$showid) {
  ...
  if (($st==ST_FREE) || ($st==ST_DELETED)) { // seat still available
    ...
    if (!mysql_query("delete from seat_locks where seatid=".$seat['id']." and showid=$showid and (until<".$now." or sid='".mysql_real_escape_string(session_id())."')")) {
      myboom("Failed deleting seat lock");
    }
    mysql_query("insert into seat_locks (seatid,showid,sid,until) values (".$seat['id']." , $showid,'".mysql_real_escape_string(session_id())."',".($now+$lockingtime).")");
  ...
  if ($seat["row"]!=-1) return $res; // (there are no equivalent
    ...
    $pot = fetch_all( mysql_query( $q="select *,1 as cnt,".((int)$seat["cat"]).
  ...
  foreach ($pot as $s) {
    ...
    mysql_query("delete from seat_locks where seatid=$potid and showid=$showid and (until<$now or sid='".mysql_real_escape_string(session_id())."')");
    
    mysql_query("insert into seat_locks (seatid,showid,sid,until) values ($potid , $showid,'".mysql_real_escape_string(session_id())."',".($now+$lockingtime).")");

    if (mysql_affected_rows()!=1) continue; // seat was locked. Try next
    /*     echo "..success"; */
...
function unlock_seats($freesession = true) {
  ...
  //  if (isset($_SESSION["seats"])) {
    ...
    mysql_query("delete from seat_locks where sid='".mysql_real_escape_string(session_id())."'");
    //    }
    //  }
  if ($freesession)
