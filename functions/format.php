...
function show_head($wide = false,$half=false) {
  global $lang,$stylesheet,$websitename;
  /* Using a proper http header rather than an http-equiv tag because
  some servers set the content-type http header to UTF8 by default
  (which overrides the http-equiv tag) */
...
/** Return a strinng with all messages, separated by newlines, and
...
function close_head($wide = false,$bodyparm = "") {
  global $titlepage;
  if ($bodyparm != "") $bodyparm = " $bodyparm ";
?>
</head>
<?php

echo '<body class="'.( $wide?'wide':'narrow' ).'"'.$bodyparm.'>';
  if ($titlepage) {
      include (FS_PATH . $titlepage);
  }
  flush_messages_html();
...
if (admin_mode()) {
  echo "<a href='".FS_PATH."logoff.php'>".$lang["logout"]."</a>";
} else {
  echo $lang["login"];
?>
<input class="fine-print" name="adminpass" type="password" size="10"> <input class="fine-print" type="submit" value="&gt;"> 
<?php } ?>
</p>
</form>
</div>
...
/* Deprecated. Use the .clsX entries in the css file instead. */
// function cls2colour($cls) {
//   switch ($cls) {
...
function show_show_info($sh=null,$correctlink=true) {
  ...
  if ($correctlink) echo " [<a href='".FS_PATH."repr.php'>".$lang["change_date"]."</a>]";
}
...
function print_booked_seats($data = null,$fmt=FMT_CORRECTLINK) {
 ...
 <p></p> has appropriate spaces already */
  ...
  if (($fmt & FMT_CORRECTLINK)==FMT_CORRECTLINK)
    $result .= '<p class="main">'.sprintf($lang["change_seats"],'[<a href="'.FS_PATH.'seats.php">','</a>]').'</p>';
  return $result;
}
